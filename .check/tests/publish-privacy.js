 ...
 *   ⑤ **不扫**工作区根下**不进发布件**的目录 —— 实测带本机路径的至少两处：
 *      `.backup-*` 那 7 个目录（里面 **75 处**本机路径）与 `projects/`（**3.6 GB** 宿主会话留痕）。
 *      ★ 陷阱备忘：在 JS **块注释**里写 glob 路径时，`.backup-*` 后面**不能紧跟斜杠** ——
 *        「星号 + 斜杠」这两个字符连写会**提前终止注释**，把后文变成代码
 *        （本文件 2026-09-18 实测踩到两次，`SyntaxError`；★ 连「说明这个陷阱」的文字里
 *        **也不能含这两个字符** —— 元级自指，第二次踩就是踩在这里）。
 *      ★ 理由：**发布件只由两条通道产生** —— `package_skill.py` 只打 `skills/andyngo/`；
 *        发布副本由 `.check/tmp/pub-sync.py` 的 `FILES` 白名单同步。这两条通道**都不含**上面两处
 *        ⇒ 它们**进不了发布件**，所以**不判**（本判据问的是「发布件」，不是「整台机器」）。
 *      ★ 这是**刻意的不判**，不是漏记：安全审计报告把它记成「在任何判据的扫描面之外」——
 *        描述是对的，但**不是缺陷**（那两处没有进发布件的通道）。
 *      ★ **触发条件**：若哪天它们进了发布件（改了打包面或同步白名单），**本条必须同步改**。
 *
