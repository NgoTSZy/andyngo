...

{
  ...
  # 【2026-09-18 加，W23 J27 · 把「过期的审计报告」这件事变成判据能看见的东西】
  # 来源：**ISS-154 结项时自己留的已知边界**（「报告存在了，但**仍没有判据会问它是否更新过 /
  #   是否过期**」）+ 审计报告 §六 第 1 条点名的同一个缺口。⇒ 丙类：去加判据，不是去问人。
  # 判什么：报告里**可机械验证的声称** —— F1「★ 守门人」声明引用的文件存在 · F2 该声明声称
  #   「已接进载体」⇒ 载体文本里必须有它 · F3「✅ 已完成」行里以 `.` 开头的路径存在。
  # ★ **报告不存在** ⇒ 打 `ABSENT` 并 rc=0（**正常缺席 ≠ 缺失**，CHG-358 口径 —— 报告是
  #   一次按需审计的产出，不是每轮必经件）；但**一条「★ 守门人」声明都解析不出** ⇒ rc=2 拒跑
  #   （形状变了 = 扫描面不再成立，防「空集 PASS」）。
  # ★ **不判**的写在判据文件头：报告里的**元陈述**（「没有判据会问 X」这类自然语言）·
  #   报告里**刻意标成历史记述**的读数（判它反而是假红）· §六 另外 5 条无机械判据的未覆盖。
  echo
  echo "=== REPORT-FRESHNESS ==="
  if [[ -f "$ROOT/.check/tests/report-freshness.js" ]]; then
    ( cd "$ROOT" && node .check/tests/report-freshness.js 2>&1 )
    seg
  else
